...
const Testimonials = () => {
  ...
  const [windowWidth, setWindowWidth] = useState(0); // Default to 0 to avoid server issues
          ...
          <div className="flex justify-center mt-6 space-x-2 sm:hidden">
            {Array.from({ length: testimonials.length }).map((_, index) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${index === activeIndex ? 'bg-travel-teal w-6' : 'bg-gray-300'
                  }`}
              />
            ))}
          </div>
